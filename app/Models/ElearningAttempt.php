...
class ElearningAttempt extends Model
{
    ...
    /**
     * 同一先生（doctor_numberが一致するMember全員）の中で、
     * 過去に一度でも合格した実績があるかどうか（期間は問わない）
     */
    public static function hasPassedByDoctorNumber(?string $doctorNumber, int $fallbackMemberId): bool
    {
        if (empty($doctorNumber)) {
            // doctor_numberが未登録の場合は、自分のMember単体のみで判定
            return static::where('member_id', $fallbackMemberId)->passed()->exists();
        }

        return static::whereHas('member', function ($q) use ($doctorNumber) {
                $q->where('doctor_number', $doctorNumber);
            })
            ->passed()
            ->exists();
    }
